...
//API setup
const API_KEY = "AIzaSyBTtKUiMoPigQokoe3a6MJj2LHNoZ7ZSb0";
...
const generateBotResponce = async (incomingMessageDiv) =>{
    ...
    try{
        //Featch bot responce from API
        const responce = await fetch(API_URL, requestOptions);
        const data = await responce.json()
        if(!responce.ok) throw new Error(data.error.message);

        //Extract and Display bot's responce text
        const apiResponceText = data.candidates[0].content.parts[0].text.replace(/\*\*(.*?)\*\*/g).trim();
        messageElement.innerText = apiResponceText;

        //Add bot responce to chat histrory
        chatHistory.push({
            role: "model",
...
const handleOutgoingMessage = (e)=>{
   ...
   //Create and Display user message 
    ...
    outgoingMessageDiv.querySelector(".message-text").textContent = userData.messsage;
    chatBody.appendChild(outgoingMessageDiv);
    chatBody.scrollTo({ top: chatBody.scrollHeight, behavior: "smooth"});

    //Simulate bot responce with thinking indicator after a dealy
    setTimeout(()=>{
...
messageInput.addEventListener("input",()=> {
    ...
    document.querySelector(".chat-form").style.borderRadiud = messageInput.scrollHeight > initialInputHeight ? "15px" : "32px";
})

//Handle file input chage and Preview the selected file
